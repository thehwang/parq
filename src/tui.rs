...
/// Filled by `run_explain` from a forgiving text-grep over the plan tree;
/// missing fields just mean the heuristic didn't fire / wasn't observable
/// in this particular plan.
#[derive(Debug, Clone, Default)]
struct ExplainSummary {
    /// One entry per data-source scan node (PARQUET_SCAN / READ_PARQUET).
    /// Joins produce two scans, simple queries one.
    scans: Vec<ScanInfo>,
...
struct ScanInfo {
    ...
    /// Estimated cardinality after filters. Plain EXPLAIN gives the
    /// optimizer's estimate, not the actual count (that needs ANALYZE).
...
fn run_explain(conn: &Connection, sql: &str, file: &str, query: &str) -> Option<ExplainSummary> {
    ...
    let scans = parse_scans(&text);
    let suggestions = gen_suggestions(&scans, file, query);
    Some(ExplainSummary {
        scans,
        suggestions,
        raw: text,
...
/// Tolerantly parse DuckDB's ASCII-art EXPLAIN tree. We strip box-drawing
/// chars first, then walk lines as a tiny state machine: operator labels
/// (UPPER_SNAKE_CASE) bracket sections; inside a parquet/read_parquet
/// section we collect Filters/Projections/cardinality lines.
///
/// The trickiest case the design has to survive is that DuckDB's parquet
/// scan node embeds its function name as a *value* under a `Function:`
/// label — meaning the literal text `READ_PARQUET` shows up twice per
/// scan, once as the operator title and once as the function-name field.
/// We disambiguate by tracking the previous non-blank line: when that
/// line ends with `:`, the next operator-shaped line is treated as a
/// value rather than a new operator boundary.
///
/// Cardinality has appeared in two shapes across DuckDB versions:
/// `Estimated Cardinality: 248` (older) and `~248 rows` (newer).
/// We accept both.
fn parse_scans(plan: &str) -> Vec<ScanInfo> {
    ...
    for raw in plan.lines() {
        ...
        if is_op_label && !is_field_value {
            ...
            if line.contains("PARQUET_SCAN") || line.contains("READ_PARQUET") {
        ...
        if let Some(s) = current.as_mut() {
            if let Some(rest) = line.strip_prefix("Filters:") {
                ...
                // Newer DuckDB: "~248 rows" or "~1 row" (commas tolerated).
                let trimmed = rest
                    .trim()
...
fn gen_suggestions(scans: &[ScanInfo], file: &str, query: &str) -> Vec<String> {
    let mut out = Vec::new();
...
fn render_status_bar(f: &mut ratatui::Frame, app: &App<'_>, area: Rect) {
    ...
    let default_help = " Tab next │ ␣ toggle col │ ⏎ append/drill │ ⌫ undo drill │ Q exit+print │ Esc quit │ : SQL │ e Explain │ ? help ";
...
fn render_explain(f: &mut ratatui::Frame, app: &App<'_>, area: Rect) {
    ...
    let lines: Vec<Line> = match &app.preview.explain {
        ...
        Some(e) => {
            ...
            // First line: at-a-glance summary.
            out.push(Line::from(vec![
                Span::raw(format!("{} scan(s)", e.scans.len())),
                Span::raw("  •  "),
                Span::raw(format!("~{} estimated rows", fmt_count(est_total))),
                Span::raw("  •  "),
                Span::raw(format!("{} filter(s) pushed", total_filters)),
                Span::raw("  •  "),
                Span::raw(format!("{} projection(s)", total_proj)),
            ]));
...
mod tests {
    ...
    // ── v0.6 explain panel: parse_scans + suggestions ──────────────────
    ...
    #[test]
    fn parse_scans_extracts_filters_and_projections() {
...
└───────────────────────────┘";
    ...
    #[test]
    fn parse_scans_handles_multiple_scans_for_join() {
        let plan = "\
HASH_JOIN
  PARQUET_SCAN
    Filters: a > 10
  PARQUET_SCAN
    ...
    #[test]
    fn parse_scans_empty_for_no_scan_plan() {
        assert!(parse_scans("").is_empty());
        assert!(parse_scans("┌───┐\n│ DUMMY │\n└───┘").is_empty());
    }

    #[test]
    fn parse_scans_handles_function_value_dedupe() {
...
└───────────────────────────┘";
    ...
    #[test]
    fn parse_scans_handles_tilde_cardinality_with_commas() {
        let plan = "\
PARQUET_SCAN
  Projections: id
  ~12,481 rows";
    ...
    fn gen_suggestions_filter_did_not_push() {
        ...
        let out = gen_suggestions(&[ScanInfo::default()], "x.parquet", "where .a > 18");
        assert!(
            out.iter()
    ...
    fn gen_suggestions_no_projection_pushdown_on_bare_query() {
        ...
        let out = gen_suggestions(&[ScanInfo::default()], "x.parquet", "");
        assert!(
